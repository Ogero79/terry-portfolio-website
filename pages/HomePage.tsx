...
const HomePage: React.FC = () => {
    ...
    const imageParallaxY = useTransform(aboutScrollY, [0, 1], ["-15%", "15%"]);
    ...
    const handleRefineIdea = async () => {
        ...
        if (!formData.message.trim()) { setRefineError("Please enter your idea first."); return; }
        ...
        try {
            ...
            const response = await ai.models.generateContent({ model: 'gem-1.5-flash', contents: prompt });
            const refinedText = response.text;
            if (refinedText) { setFormData({ ...formData, message: refinedText }); } 
            else { throw new Error("Received an empty response from the AI."); }
        } catch (error) { console.error("AI refinement failed:", error); setRefineError("Couldn't refine the idea right now. Please try again.");
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setSubmissionState('submitting'); setFormStatus('');
        const templateParams = {
            name: formData.name, email: formData.email, message: formData.message,
            time: new Date().toLocaleString('en-KE', { dateStyle: 'full', timeStyle: 'short', timeZone: 'Africa/Nairobi' })
        };
        ...
        .then(() => {
            setSubmissionState('success'); setFormStatus('Message Sent! Thank you.');
            setFormData({ name: '', email: '', message: '' });
        }, () => {
            setSubmissionState('error'); setFormStatus('Failed to send. Please try again.');
        })
        .finally(() => { setTimeout(() => { setSubmissionState('idle'); setFormStatus(''); }, 3000); });
    };
    ...
    return (
        <div className="overflow-x-hidden space-y-20 md:space-y-32 pb-16 md:pb-24">
            
            {/* Section 1: Hero */}
            <section className="relative min-h-screen flex items-center justify-center text-center -mb-20 md:-mb-32">
                <div className="container mx-auto px-6">
                    <motion.h1
                        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }}
                        className="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-extrabold font-display text-primary-blue max-w-5xl mx-auto leading-tight"
                    >
                    ...
                    <motion.p
                        ...
                        className="mt-6 text-base md:text-lg text-primary-blue/80 max-w-2xl mx-auto"
                    >
            ...
            <section className="container mx-auto px-6">
                <motion.h2
                    variants={fadeInAnimation} initial="initial" whileInView="animate" viewport={{ once: true }}
            ...
            <section id="about" ref={aboutSectionRef} className="container mx-auto px-6 scroll-mt-24">
                <div className="grid md:grid-cols-2 gap-12 md:gap-24 items-center">
                    ...
                    <motion.div
                        variants={fadeInAnimation} initial="initial" whileInView="animate" viewport={{ once: true, amount: 0.5 }}
                    >
            ...
            {/* Section 4: Contact */}
            <section id="contact" className="container mx-auto px-6 text-center scroll-mt-24">
                 <motion.h2
                    variants={fadeInAnimation} initial="initial" whileInView="animate" viewport={{ once: true }}
                ...
                <div className="mt-16 flex justify-center space-x-6">
                    ...
                    <a href="https://www.instagram.com/terrydesigns21?igsh=M2twbnRvc2NyZWpn" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-primary-blue hover:text-accent-gold transition-colors"><InstagramIcon className="h-8 w-8" /></a>
